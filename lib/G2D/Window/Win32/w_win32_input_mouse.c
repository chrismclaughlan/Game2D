...
void 
window_input_mouse_event_move(int x, int y)
{
...
void 
window_input_mouse_event_scroll(int delta)
{
	gp_g2d_input->mouse_scroll += delta;
}

void 
window_input_mouse_event_click(uint vk_code, bool is_down)
{
	switch (vk_code)
	{
...
bool 
window_input_mouse_is_within_rect(int x0, int y0, int x1, int y1)
{
	return (gp_g2d_input->mouse_pos.x > x0 && gp_g2d_input->mouse_pos.y > y0 && gp_g2d_input->mouse_pos.x <= x1 && gp_g2d_input->mouse_pos.y <= y1);
}

struct Vec2f 
window_input_mouse_get_screen_coords(void)
{
	return r_px_to_screen(gp_g2d_input->mouse_pos);
}