...
void game_render_object(struct Object *object)
{
	for (struct Line *line = object->lines_start; line != NULL; line = line->next)
	{
		...
		const struct Vec2f start_offset = {
			line->start.x + object->offset_x,
			line->start.y + object->offset_y};
		const struct Vec2f end_offset = {
			line->end.x + object->offset_x,
			line->end.y + object->offset_y};
		r_draw_line_f(line->colour, start_offset, end_offset, false);
	}
}
...
void game_render_los(const struct Vec2f *vf_origin)
{
	...
	{
		if (j == g_scene_objects_intersected_queue_index)
			j = 0;
